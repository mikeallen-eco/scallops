...
```
# Plot relationship between % present and climate variable
```{r}
...
```
# Define the model (quadratic effect of climatic variable)
```{r}

cat(file = "scripts/scal_dyn_occ_quad.txt", 
    "
model{
  for(i in 1:nSites) {
    # biological process
...
")
```
# Bundle data for JAGS - quadratic mean SBT model
```{r}
...
jdata_meansbt <- list(nSites = nrow(ymat), nYears = ncol(ymat), y = ymat,
    n = nmat, z = z, climvar = meansbt_matS,
    climvar2 = meansbt_matS^2)
str(jdata_meansbt)

wanted <- c("psiInt", "phiInt", "gamInt", "p", "bpsi", "b2psi", "bphi", "b2phi", "bgam", "b2gam", "N", "gamma", "phi")

# 10 mins
( out_meansbt_quad <- jagsUI::jags(jdata_meansbt, NULL, wanted, 
                      "scripts/scal_dyn_occ_quad.txt",
    n.chains=3, n.iter=10000, n.adapt=1000, DIC=T, parallel=TRUE) )

saveRDS(out_meansbt_quad, "C:\\Users\\Mike\\Desktop\\scallop_dynocc_plots/out_meansbt_quad.rds")

```
# plot effects - quadratic mean SBT model
```{r}
plot(out_meansbt_quad$mean$N, type = "l")

range(meansbt_mat)  #  3.983414 18.407284
cx <- seq(4, 18.4, length=101)
cxS <- wiqid::standardize2match(cx, meansbt_mat)
cxS2 <- cxS^2

params <- out_meansbt_quad$mean

lpsix <- with(params, qlogis(psiInt) + bpsi * cxS + b2psi*cxS2)
psix <- plogis(lpsix)
...
for(i in 1:37) {
  lphix[,i] <- with(params, qlogis(phiInt[i]) + bphi * cxS +
                      b2phi * cxS2)
phix <- plogis(lphix)
...
for(i in 1:37){
  lgamx[,i] <- with(params, qlogis(gamInt[i]) + bgam * cxS +
                      b2gam * cxS)
gamx <- plogis(lgamx)
...
saveRDS(out_meansbt_gaus, "C:\\Users\\Mike\\Desktop\\scallop_dynocc_plots/out_meansbt_gaus3.rds")

```
# plot effects - Gaussian mean SBT model
```{r}
out_meansbt_gaus <- readRDS(
  "C:\\Users\\Mike\\Desktop\\scallop_dynocc_plots/out_meansbt_gaus3.rds")
