...
#   filter(between(lat_floor, 35,44)) %>% 
...
#   coord_flip()
# gglength  
# ggsave(gglength, filename=here("results","scallop_length_freq.png"))
...
# ggpatchlength  
# ggsave(ggpatchlength, filename=here("results","scallop_length_freq_by_patch.png"), height=8, width=5, dpi=160)
...
f = 0.45 #F7-8 in 2017 equaled 0.45 ... and ranged from 0.13 in 1965 to 1.04 in 1975 (Figure B1).
z = exp(-m-f)
age_at_maturity = 4
t0=-.2
cv= 0.2 # guess
...
# fit model
######
# stan_data <- readRDS("results/scallop_stan_data_122021.rds")
# stan_data <- readRDS("results/scallop_stan_data_d0_01042022.rds")
stan_data <- list(
  np=np,
  n_ages=n_ages,
  ny_train=ny,
...
# stan_model_fit <- readRDS("results/scallop_stan_fit_122021.rds")
# stan_model_fit <- readRDS("results/scallop_stan_fit_122021.rds")
...
# ggsave("results/abund_p_y_hat.png", dpi = 400, height = 10, width = 10)

# assess length comp fits

n_p_l_y_hat <- tidybayes::gather_draws(stan_model_fit, n_p_l_y_hat[year,patch,length], n = 500) # originally
...
dat_train_lengths %>% 
  ...
  geom_point()
# ggsave("results/n_by_patch_year.png", dpi = 400, height = 4, width = 6)
...
n_p_l_y_hat %>% 
 ...
 # ggsave("results/n_p_l_y_hat.png", dpi = 400, height = 10, width = 10)
...
saveRDS(stan_model_fit, here("results","scallop_stan_fit_122021.rds"))
...
ggsave("results/centroid_shift_122021.png", dpi = 400, width = 6, height = 4)
...
abund_p_y %>% 
  ...
  labs(title="Observed")
# ggsave("results/abund_p_y.png", dpi = 400, width = 6, height = 4)
...
# ggsave("results/abund_p_y_estimated.png", dpi = 400, width = 6, height = 4)
...
dat_train_lengths %>% 
  ...
  facet_wrap(~patch)
# ggsave("results/who_is_colonizing.png", dpi = 400, width = 9, height = 7)
...
dat_train_lengths %>% 
  ...
  scale_fill_viridis_c() +
  theme_bw()
# ggsave("results/sum_num_at_length.png", dpi = 400, width = 9, height = 7)
...
dat_train_lengths %>% 
  ...
  scale_fill_viridis_c() +
  theme_bw()
# ggsave("results/sum_num_at_length2.png", dpi = 400, width = 9, height = 7)
