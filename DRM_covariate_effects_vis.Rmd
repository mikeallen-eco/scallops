...
# attempt at mortality with rowwise posterior calcs
ta.ts_mort <- matrix(NA, nrow = 600, ncol = nrow(dat_dens80))
temp_adjust <- matrix(NA, nrow = 600, ncol = nrow(dat_dens80))
O2strat_adjust <- matrix(NA, nrow = 600, ncol = nrow(dat_dens80))

for(i in 1:nrow(dat_dens80)){ # 1 col for each row of dat_dens80, 1 row for each iteration
  for(j in 1:600){
temp_adjust[i,j] <- exp(-0.5 * ((dat_dens80$climvar_mort[i] - 
                          rm$Topt_mort[j])/rm$width_mort[j])^2)

O2strat_adjust[i,j] <- plogis(rm$linbeta0mort[j] + 
                          rm$O2betamort[j]*O2),
         ta = temp_adjust*O2strat_adjust*exp(-0.35)
         ) %>%
  group_by(year) %>%
...
# calculate survival by cell and year
