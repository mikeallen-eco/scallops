...
```
# Plot change in 0.5 grid cell average catch by time period
...
```
# Plot change in 0.5 grid cell average SBT by time period
```{r}
# summarize change in mean catch between 1980-1999 and 2000-2015
change_sbt = hd %>%
  st_drop_geometry() %>%
  ...
  filter(change < 15) %>% # removes 1 very unusual coastal point
  st_as_sf(coords = c("grid_lon", "grid_lat"),
           crs = 4326) 
...
ggplot() +
  ...
  # annotate("text", x = change_sbt$lon, y = change_sbt$lat, 
  #          label = round(change_sbt$change,1),
  #          size = 4) +
  scale_fill_gradient2(midpoint = 0, 
                       low = "steelblue", 
                       mid = "white",
