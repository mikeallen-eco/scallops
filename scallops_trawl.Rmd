...
```
# Plot CHANGE in average SBT by time period (0.5 grid cell)
```{r}
change_clim <- clim %>% 
...
```{r}
clim %>%
  filter(grid %in% filter(change_clim, change < 10)$grid) %>%
ggplot() + geom_line(aes(x = year, y = mean_temp, color = grid)) + guides(color = "none")
...
ggplot(clim) + geom_line(aes(x = year, y = O2, color = grid)) + guides(color = "none")


matchup <- change_clim %>%
  st_drop_geometry() %>%
  as.data.frame() %>%
  left_join(as.data.frame(change_sbt), by = c("grid", "lat", "lon"))

ggplot(matchup) +
  geom_point(aes(x = change.x, y = change.y))
```
