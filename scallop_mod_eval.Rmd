...
null_h <- readRDS("D:/scallop/stan_model_posts_run20221216a.rds")
rminstrat_mmaxO2 <- 
  readRDS("D:/scallop/stan_model_posts_run20221214a.rds")
rmeanstrat_mmeanO2 <- 
  readRDS("D:/scallop/stan_model_posts_run20221201a.rds")

# rmmeanO2 <- readRDS("results/stan_model_posts_run20221116c.rds")
# rmean <- readRDS("results/stan_model_posts_run20221012a.rds")
# rmean_len <- readRDS("results/stan_model_posts_run20221031a.rds")
# rmean_both <- readRDS("results/stan_model_posts_run20221110a.rds")
...
```
# null_h model: no covariates on rec or surv; half degree res
```{r}
# visualize observed vs. predicted projections of 80mm+ scallops
mod <- null_h 
mod <- rminstrat_mmaxO2
mod <- rmeanstrat_mmeanO2

pred80 <- cbind(apply(mod$dens_p_y_hat80, 
                               c(2,3), median)) %>%
  t() %>%
  as.data.frame() %>%
  mutate(year = 1983:2014) %>%
  pivot_longer(cols = V1:V116) %>%
  mutate(patch = as.numeric(substr(name, 2,4))) %>%
  arrange(patch, year) %>%
  select(year, patch, est80 = value) %>%
  mutate(est80 = est80)

obs80 <- dat_train_dens80 %>%
  arrange(patch, year) %>%
  mutate(year = year + 1982,
         obs80 = mean_dens * meanpatcharea)

obspred <- obs80 %>%
  full_join(pred80, by = c("year", "patch")) %>%
  arrange(patch, year)

# calculate spearmans correlation
obspred_cor <- obspred %>% filter(is.na(obs80)==F)
cor(obspred_cor$obs80, 
    obspred_cor$est80, 
    method = "spearman") 
# 0.162 for null (half degree)
# 0.581 for min/max model
# 0.512 for mean/mean model
...
obspred %>%
  ...
  ggplot(aes(x = wiqid::standardize(log10((obs80/meanpatcharea)+1)), 
             y = wiqid::standardize(log10((est80/meanpatcharea)+1)),
             color = year)) +
  geom_point(size = 4,
             alpha = .4) +
  geom_smooth(method = "lm", se = F,
              color = "black") +
  # annotate("text", x = 500, y = 6, label = "Spearmans =\n0.162") +
  ...
  # scale_y_log10() +
  # scale_x_log10() +
  scale_color_continuous(breaks = seq(1983,2013,length.out = 4)) +
  theme_bw() +
  theme(text = element_text(size = 14)) 

ggsave("writing/v0/obspred_rmeanstrat_mmeanO2.png",
...
# rmmeanO2 model: Rec & Survival vs. Mean SBT & O2 - change in 10 year mean pop. est, observed; also suitability
```{r}
...
pred80_rmmeanO2 <- cbind(apply(rmmeanO2$proj_dens_p_y_hat80, 
                               c(2,3), median)) %>%
  t() %>%
  as.data.frame() %>%
...
obs80_rmmeanO2 <- dat_test_dens80 %>%
...
obspred_rmmeanO2 <- obs80_rmmeanO2 %>%
  full_join(pred80_rmmeanO2, by = c("year", "patch")) %>%
...
# calculate spearmans correlation
cor(filter(obspred_rmmeanO2, is.na(obs80)==F)$obs80, 
    filter(obspred_rmmeanO2, is.na(obs80)==F)$est80, 
    method = "spearman")
...
Tpred80_rmmeanO2 <- cbind(apply(rmmeanO2$dens_p_y_hat80, 
                             c(2,3), median)) %>%
  t() %>%
  as.data.frame() %>%
...
Tobs80_rmmeanO2 <- dat_train_dens80 %>%
  arrange(patch, year) %>%
  mutate(year = year + 1979,
         obs80 = mean_dens * meanpatcharea)

Tobspred_rmmeanO2 <- Tobs80_rmmeanO2 %>%
  full_join(Tpred80_rmmeanO2, by = c("year", "patch")) %>%
  mutate(Lest80 = lag(est80),
         Lobs80 = lag(obs80),
         est_lambda = est80/Lest80) %>%
  arrange(patch, year)
...
# calculate spearmans correlation
cor(filter(Tobspred_rmmeanO2, is.na(obs80)==F)$obs80, 
    filter(Tobspred_rmmeanO2, is.na(obs80)==F)$est80, 
    method = "spearman")
...
obspred_rmmeanO2_by_grid <- obspred_rmmeanO2  %>%
  filter(year != 2004,
         is.na(mean_dens)==F) %>%
  group_by(grid) %>%
...
state_bounds <- rnaturalearth::ne_coastline(scale = 10, returnclass = "sf") %>%
  st_crop(obspred_rmmeanO2_by_grid)

obspred_rmmeanO2_by_grid %>%
...
ggsave("figures/rmmeanO2/projected_loglambda.png", 
       height = 6, width = 6, dpi = 400)

# get model predicted values from training set
pred80_rmmeanO2_train <- cbind(apply(rmmeanO2$dens_p_y_hat80, 
                                  c(2,3), median)) %>%
  t() %>%
  as.data.frame() %>%
...
obs80_rmmeanO2_train <- dat_train_dens80 %>%
  arrange(patch, year) %>%
  mutate(year = year + 1979,
         obs80 = mean_dens * meanpatcharea)

obspred_rmmeanO2_train <- obs80_rmmeanO2_train %>%
  full_join(pred80_rmmeanO2_train, by = c("year", "patch")) %>%
  mutate(Lest80 = lag(est80),
         Lobs80 = lag(obs80),
         est_lambda = est80/Lest80) %>%
  arrange(patch, year)
...
test = obspred_rmmeanO2_train %>% 
  group_by(grid) %>% 
  summarize(presn = sum(obs80>0))
test2 = obspred_rmmeanO2 %>% 
  group_by(grid) %>% 
  summarize(presn = sum(obs80>0))

# create period 1
obspred_rmmeanO2_per1 <- obspred_rmmeanO2_train  %>%
  filter(year %in% 1995:2004,
         is.na(mean_dens)==F) %>%
  mutate(suit = 
           exp(-0.5 * ((climvar - 
                          median(rmmeanO2$Topt))/median(rmmeanO2$width))^2)) %>%
  group_by(grid) %>%
...
# create period 2
obspred_rmmeanO2_per2 <- obspred_rmmeanO2  %>%
...
obspred_rmmeanO2_change <- obspred_rmmeanO2_per1 %>%
  st_join(obspred_rmmeanO2_per2) %>%
...
ggsave("figures/rmmeanO2/obsdens_per1.png", 
       height = 6, width = 6, dpi = 400)
...
ggsave("figures/rmmeanO2/obsdens_per2.png", 
       height = 6, width = 6, dpi = 400)

# map observed change in scallop abundance per 1-2
(obs80_chg_plot <- 
    obspred_rmmeanO2_change %>%
...
ggsave("figures/rmmeanO2/obsdens_change.png", 
       height = 6, width = 6, dpi = 400)
...
ggplot(obspred_rmmeanO2_change, aes(x = obs80_per2+1, 
                                 y = est80_per2+1)) +
  geom_point(size = 3,
             alpha = .7) +
...
cor(obspred_rmmeanO2_change$obs80_per2,
    obspred_rmmeanO2_change$est80_per2,
    method = "spearman") # 0.86

ggplot(obspred_rmmeanO2_change) +
  geom_point(size = 3,
...
ggplot(obspred_rmmeanO2_change, aes(x = suit_chg, 
                                 y = obs80_chg)) +
