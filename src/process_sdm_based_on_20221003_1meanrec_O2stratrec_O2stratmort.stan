...
if(T_dep_mortality == 1){
  ...
  for (y in 2:ny_train){
    ...
    for(p in 1:np){
      ...
      if(T_dep_recruitment==1 && spawner_recruit_relationship==0){
        n_p_a_y_hat[p,1,y] = mean_recruits * exp(rec_dev[y-1] - pow(sigma_r,2)/2) * T_adjust_rec[p,y-1];
...
model {
  ...
  if(spawner_recruit_relationship==0){
    ...
    // for(i in 1:np){
    log_mean_recruits ~ normal(7,5); // was norm(7,3) or norm(7,5)
    // }
  }
...
generated quantities {
  ...
  for(p in 1:np){
    for(y in 1:ny_proj){
      T_adjust_rec_proj[p,y] = T_dep_rec(sbt_rec_proj[p,y], Topt_rec, width_rec) * inv_logit(linbeta0rec + O2betarec*O2_proj[p,y] + stratbetarec*strat_proj[p,y]);
    } // close years
  } // close patches
  
  for(p in 1:np){
    for(y in 1:ny_proj){
      T_adjust_mort_proj[p,y] = T_dep_mort(sbt_mort_proj[p,y], Topt_mort, width_mort) * inv_logit(linbeta0mort + O2betamort*O2_proj[p,y] + stratbetamort*strat_proj[p,y]);
    } // close years
  } // close patches
  ...
  for(y in 2:(ny_proj+1)){
    for(p in 1:np){
      ...
      if(T_dep_recruitment==1){
        proj_n_p_a_y_hat[p,1,y] = mean_recruits * exp(rec_dev_proj[y-1] - pow(sigma_r,2)/2) * T_adjust_rec_proj[p,y-1];
      }
