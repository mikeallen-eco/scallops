functions {
  ...
  real T_dep_rec(real sbt, real Topt_rec, real width_rec){
    return exp(-0.5 * ((sbt - Topt_rec)/width_rec)^2); // gaussian temperature-dependent function
  }
  
  real T_dep_mort(real sbt, real Topt_mort, real width_mort){
    return exp(-0.5 * ((sbt - Topt_mort)/width_mort)^2); // gaussian temperature-dependent function
  }
...
parameters{
  ...
  real O2beta0rec; // intercept for logistic O2 function vs. recruitment
  
  real O2betarec; // slope for logistic O2 function vs. survival
  
  real O2beta0mort; // intercept for logistic O2 function
  
  real O2betamort; // slope for logistic O2 function
...
if(T_dep_recruitment == 1){
  ...
  for(p in 1:np){
    for(y in 1:ny_train){
      T_adjust_rec[p,y] = T_dep_rec(sbt[p,y], Topt_rec, width_rec) * inv_logit(O2beta0rec + O2betarec*O2[p,y]);  
...
if(T_dep_mortality == 1){
  ...
  for(p in 1:np){
    for(y in 1:ny_train){
      T_adjust_mort[p,y] = T_dep_mort(sbt[p,y], Topt_mort, width_mort) * inv_logit(O2beta0mort + O2betamort*O2[p,y]);  
      // print("T_adjust in patch", p, " and year ",y," is ",T_adjust[p,y]);
...
model {
  ...
  O2beta0rec ~ normal(0, 1.4);

  O2betarec ~ normal(0, 5);
  
  O2beta0mort ~ normal(0, 1.4);
...
generated quantities {
  ...
  for(p in 1:np){
    for(y in 1:ny_proj){
      T_adjust_rec_proj[p,y] = T_dep_rec(sbt_proj[p,y], Topt_rec, width_rec) * inv_logit(O2beta0rec + O2betarec*O2[p,y]);
    } // close years
  } // close patches
  
  for(p in 1:np){
    for(y in 1:ny_proj){
      T_adjust_mort_proj[p,y] = T_dep_mort(sbt_proj[p,y], Topt_mort, width_mort) * inv_logit(O2beta0mort + O2betamort*O2[p,y]);
    } // close years
  } // close patches
