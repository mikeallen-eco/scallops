...
parameters{
  ...
  vector[ny_train] log_mean_recruits; // year-specific mean recruits (one value across all patches)
...
transformed parameters{
  ...
  //real mean_recruits;
  real mean_recruits[np, ny_train];
  ...
  for(p in 1:np){
    for(y in 1:ny_train){
  mean_recruits[p,y] = exp(log_mean_recruits[y] + O2betarec*O2[p,y] + stratbetarec*strat[p,y]);
    }
  }
...
if(T_dep_recruitment == 1){
  ...
  for(p in 1:np){
    for(y in 1:ny_train){
      T_adjust_rec[p,y] = T_dep_rec(sbt_rec[p,y], Topt_rec, width_rec);  
...
if(T_dep_mortality == 1){
  ...
  for(p in 1:np){
    for(y in 1:ny_train){
      T_adjust_mort[p,y] = T_dep_mort(sbt_mort[p,y], Topt_mort, width_mort) * inv_logit(linbeta0mort + O2betamort*O2[p,y] + stratbetamort*strat[p,y]);  
      // print("T_adjust in patch", p, " and year ",y," is ",T_adjust[p,y]);
    ...
    for(a in 1:n_ages){
      if(a==1){
        ...
        if(T_dep_recruitment==1 && spawner_recruit_relationship==0){
          ...
          n_p_a_y_hat[p,a,1] = mean_recruits[p,1] * T_adjust_rec[p,1] * exp(raw[1] - pow(sigma_r,2) / 2); // initialize age 0 with mean recruitment in every patch
        }
        if(T_dep_recruitment==0 && spawner_recruit_relationship==0){
          ...
          n_p_a_y_hat[p,a,1] = mean_recruits[p,1] * exp(raw[1] - pow(sigma_r,2) / 2); // initialize age 0 with mean recruitment in every patch
        }
  ...
  for (y in 2:ny_train){
    ...
    for(p in 1:np){
      ...
      if(T_dep_recruitment==1 && spawner_recruit_relationship==0){
        ...
        n_p_a_y_hat[p,1,y] = mean_recruits[p,y-1] * exp(rec_dev[y-1] - pow(sigma_r,2)/2) * T_adjust_rec[p,y-1];
      }
      if(T_dep_recruitment==0 && spawner_recruit_relationship==0){
        //n_p_a_y_hat[p,1,y] = mean_recruits * exp(rec_dev[y-1] - pow(sigma_r,2)/2) ;
        n_p_a_y_hat[p,1,y] = mean_recruits[p,y-1] * exp(rec_dev[y-1] - pow(sigma_r,2)/2) ;
      }
...
generated quantities {
  ...
  for(p in 1:np){
    for(y in 1:ny_proj){
      T_adjust_mort_proj[p,y] = T_dep_mort(sbt_mort_proj[p,y], Topt_mort, width_mort) * inv_logit(linbeta0mort + O2betamort*O2[p,y] + stratbetamort*strat[p,y]);
    } // close years
  } // close patches
  ...
  for(y in 2:(ny_proj+1)){
      ...
      if(T_dep_recruitment==1){
        proj_n_p_a_y_hat[p,1,y] = mean_recruits[p,y] * exp(rec_dev_proj[y-1] - pow(sigma_r,2)/2) * T_adjust_rec_proj[p,y-1];
      }
      if(T_dep_recruitment==0){
        proj_n_p_a_y_hat[p,1,y] = mean_recruits[p,y] * exp(rec_dev_proj[y-1] - pow(sigma_r,2)/2);
      }
